    ...
    @foreach ($anonymousPosts as $item)
        <div class="comment-view">
            ...
            <div class="view-more mt-0">
                ...
                @endisset
                <form id="postFormAnonymous" action="{{ route('group.comment', $item->id) }}" method="POST">
                    @csrf
                    <div class="post-something">
                        <div class="comment-dot mr-2"></div>
                        <input type="text" id="postInputAnonymous" name="content" value="{{ old('content') }}"
...
<script>
    document.getElementById('postInputAnonymous').addEventListener('keypress', function(event) {
        if (event.key === 'Enter') {
            event.preventDefault();
            document.getElementById('postFormAnonymous').submit(); // Submit the form
        }
    });
</script>
<style>
